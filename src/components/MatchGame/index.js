...
class MatchGame extends Component {
  ...
  changeTab = id => this.setState({category: id})

  render() {
    const {tabList, imageList, category, index, timer} = this.state
    const filteredList = imageList.filter(
      eachThumbnail => eachThumbnail.category === category,
    )

    /* const index = Math.ceil(Math.random() * (imageList.length - 1)) */
        ...
        <div className="game-section-container">
          ...
          <ul className="thumbnail-list-container">
            {filteredList.map(eachImage => (
              <ThumbnailItem thumbnailItem={eachImage} key={eachImage.id} />
            ))}
          </ul>
        </div>
      </div>
